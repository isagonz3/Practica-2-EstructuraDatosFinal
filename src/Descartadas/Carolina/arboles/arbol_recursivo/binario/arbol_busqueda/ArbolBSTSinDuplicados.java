...
public class ArbolBSTSinDuplicados<T extends Comparable<T>> extends ArbolBSTBase<T> { //árbol binario de búsqueda sin permitir valores repetidos
    ...
    private NodoBinario<T> insertarRec(NodoBinario<T> nodo, T data) { //inserción típica de un bst
        ...
        } //si es igual no se inserta porque no se permiten duplicados
    ...
    private NodoBinario<T> buscarRec(NodoBinario<T> nodo, T data) { //búsqueda en un bst
    ...
    private NodoBinario<T> eliminarRec(NodoBinario<T> nodo, T data) { //eliminación en un bst
        ...
        if (cmp < 0) { //si es menor se busca en la izquierda
            ...
            //si tiene dos hijos se busca el sucesor (el menor del subárbol derecho)
            NodoBinario<T> sucesor = min(nodo.getRight());

            nodo.setData(sucesor.getData()); //se reemplaza el valor del nodo por el del sucesor
    ...
    private NodoBinario<T> min(NodoBinario<T> nodo) { //busca el nodo con menor valor en un subárbol
